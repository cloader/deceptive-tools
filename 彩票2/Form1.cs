    ...
    public partial class Form1 : CCSkinMain
    {
...

        {
            ...
            foreach (List<int> item in Clone(result))
            {
                ...
                log_textbox.AppendText(System.Environment.NewLine + String.Join(",", item) + "  AC值" + acval);
                bool isok = false;
            ...
            {
                int sum = item.Sum();
                log_textbox.AppendText(System.Environment.NewLine + String.Join(",", item) + "  和值" + sum);
            ...
            foreach (List<int> item in Clone(result))
            {
                ...
                log_textbox.AppendText(System.Environment.NewLine + String.Join(",", item) + "  奇偶比" + jo_key);
                bool isok = false;
            ...
            foreach (List<int> item in Clone(result))
            {
                ...
                log_textbox.AppendText(System.Environment.NewLine + String.Join(",", item) + "  大小比" + dx_key);
                bool isok = false;
            ...
            foreach (List<int> item in Clone(result))
            {
                ...
                String lh_key = v + "连号";
                bool isok = false;
                log_textbox.AppendText(System.Environment.NewLine + String.Join(",", item) + "  连号" + lh_key);
            ...
            for (int i = 0; i < result.Count; i++)
            {
                if (i > 0 && i % 4 == 0)
                {
                    result_textbox.Text = result_textbox.Text + System.Environment.NewLine;
                }
                result_textbox.Text = result_textbox.Text + String.Join(",", result.ElementAt(i)) + "   ";
